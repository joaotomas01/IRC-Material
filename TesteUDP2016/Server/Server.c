...
int main(int argc, char* argv[])
{
	...
	double receivedValue, total;
	int nReceivedMessages;
	...
	while (1) {
		...
		while (nReceivedMessages < MAX_MSGS && toContinue) {
			...
			if (nbytes == SOCKET_ERROR) {
				...
				if (WSAGetLastError() == WSAETIMEDOUT)
					...
					Abort((char*)"Erro na recepcao de datagramas");
				...
				sourceMsgs[nReceivedMessages] = cli_addr;
		...
		sprintf_s(response, sizeof(response), "Number of received messages: %d ; Sum of values: %f", nReceivedMessages, total);
		...
		for (i = 0; i < nReceivedMessages; i++) {
			...
			nbytes = sendto(s, response, sizeof(response), 0, (struct sockaddr*)&sourceMsgs[nReceivedMessages], sizeof(sourceMsgs[nReceivedMessages]));
