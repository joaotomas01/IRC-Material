...
int main(int argc, char* argv[])
{
	...
	while (1) {
		...
		recvCmd = strtok_s(buffer, " ", &next_token);

		printf("%s\n", recvCmd);
		if (recvCmd != NULL) {
			if (strcmp(recvCmd, "log") == 0) {
				recvCmd = strtok_s(NULL, " ", &next_token);
				while (recvCmd != NULL) {
					if (nPar == 0) {
						strcpy_s(usersList[usersListSize], strlen(recvCmd) + 1, recvCmd);
						recvCmd = strtok_s(NULL, " ", &next_token);
						nPar++;
					}
					else {
				...
				if (nPar > 0) {
					printf("Argumentos insuficientes: log <nome> <idade>\n");
					continue;
				}
			}
			else if (strcmp(recvCmd, "exit") == 0) {
				Abort("Cliente pediu para terminar");
			}
			else if (strcmp(recvCmd, "list") == 0) {
				break;
			}
			else if (strcmp(recvCmd, "media") == 0) {
				break;
			}
		}
